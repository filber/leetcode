            ...
            while (
                // 1. could pop
                    ...
                    top >= 0 // stack is not empty
                            ...
                            && curLetter - (stack[top] == letter ? 1 : 0) + letterCount >= repetition // have enough letters
                    )
                ||
                // 2. must pop
                curLetter + k - top - 1 < repetition // no enough position for letter
            ) {
                char peek = stack[top];
                top--;
                curLetter -= peek == letter ? 1 : 0;
            }

            if (top + 1 < k) {
                curLetter += arr[i] == letter ? 1 : 0;
                stack[++top] = arr[i];
            }
