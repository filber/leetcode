public class _1901_FindAPeakElementII {
    public static int[] findPeakGrid(int[][] mat) {
        ...
        if (m <= n) {
            // search by row
            for (int i = 0; i < m; i++) {
                int peakIndex = findPeakHorizontally(mat,i,0,n-1);
                if (judge(mat, i, peakIndex)) {
                    return new int[]{i, peakIndex};
                }
            }
        } {
            // search by column
            for (int i = 0; i < n; i++) {
                int peakIndex = findPeakVertically(mat,i,0,m-1);
                if (judge(mat, peakIndex, i)) {
                    return new int[]{peakIndex, i};
                }
            }
        }
        return null;
    ...
    public static int findPeakHorizontally(int[][] mat, int row, int start, int end) {
        int n = mat[0].length;
        if (start == end) {
            return start;
        }
        ...
        int middleValue = mat[row][middleIndex];
        int left = middleIndex == 0 ? -1 : mat[row][middleIndex - 1];
        int right = middleIndex == n - 1 ? -1 : mat[row][middleIndex + 1];

        if (middleValue > left && middleValue > right) {
            return middleIndex;// found
        } else if (middleValue > left) {
            // find in the right part
            return findPeakHorizontally(mat, row, middleIndex + 1, end);
        } else {
            // find in the left part
            return findPeakHorizontally(mat, row, start, middleIndex);
        }
    }

    public static int findPeakVertically(int[][] mat, int col, int start, int end) {
        int m = mat.length;
        if (start == end) {
            return start;
        }
        ...
        int middleIndex = (start + end) / 2;
        int middleValue = mat[middleIndex][col];
        int top = middleIndex == 0 ? -1 : mat[middleIndex - 1][col];
        int bottom = middleIndex == m - 1 ? -1 : mat[middleIndex + 1][col];

        if (middleValue > top && middleValue > bottom) {
            return middleIndex;// found
        } else if (middleValue > top) {
            // find in the bottom part
            return findPeakVertically(mat, col, middleIndex + 1, end);
        } else {
            // find in the top part
            return findPeakVertically(mat, col, start, middleIndex);
        }
    }
